...
void loop()
{
  ...
  if (loop_count == 100000) {
    unsigned long send_count = controller.send_count();
    unsigned long recv_count = controller.recv_count();
    M5.Lcd.printf("send : %lu \n", send_count);
    M5.Lcd.printf("recv : %lu \n", recv_count);
    M5.Lcd.printf("drop : %lu \n", send_count - recv_count);
    M5.Lcd.printf("loou count : %lu \n", loop_count);
    M5.Lcd.printf("error rate : %f \n", (float)(send_count - recv_count) / send_count * 100.0f );
    M5.Lcd.printf("time       : %f \n", (float)(millis()-start) / loop_count );
  }
  loop_count++;
}
